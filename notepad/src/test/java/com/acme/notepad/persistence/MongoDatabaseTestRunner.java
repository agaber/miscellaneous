...
/**
 * Launches a single mongod process for all tests that use this test Runner on
 * port 10000. The mongod process will be shared for all tests so it is up to
 * the tester not to rely on this shared state. Don't rely on a clean database
...
public class MongoDatabaseTestRunner extends BlockJUnit4ClassRunner {
  ...
  private synchronized void mongod() {
    // Only one mongod at any given time.
    if (mongod != null) {
      return;
    }

    // Clean up when all tests are complete.
